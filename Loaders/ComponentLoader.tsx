...
export const isIComponentLoader: (toTest: IComponentLoader | IComponentLoaderType) => toTest is IComponentLoader = (
  toTest: IComponentLoader | IComponentLoaderType,
): toTest is IComponentLoader => {
...
export class ComponentLoader {
   ...
   */
  public getPreLoadedType<P = ComponentProps<IContent>>(
    component: string,
    throwOnUnknown = true,
  ...
  public LoadType<P = ComponentProps<IContent>>(component: string): Promise<ComponentType<P>> {
    if (this.isPreLoaded(component)) return Promise.resolve(this.getPreLoadedType(component) as ComponentType<P>);
  ...
  protected doLoadComponentType(component: string): Promise<ComponentType> {
    ...
    const tryOption = (idx: number) =>
      new Promise<ComponentType>((resolve, reject) => {
        options[idx]
          .load(component)
          .then((c) => {
  ...
  public async LoadComponent<P = ComponentProps<IContent>>(
    component: string,
    props: P,
  ): Promise<React.ReactElement<P>> {
